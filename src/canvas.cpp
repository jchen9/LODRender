...
void render_logo(int id)
{
    ...
    if(logo_texture_info == NULL)
    {
		...
		logoPath = getFilePathInBundle("core_logo", "png");
#else
		const int kBufferSize = 256;
		logoPath = (char *) malloc(kBufferSize);

		strcpy(logoPath, "./resources/core_logo.png");
#endif
...
void render_canvas(int id)
{
    ...
    glPushMatrix();
    {
        ...
        if(showTieDepth)
        {
    	    ...
    	    float bigger = (c->coverage_x > c->coverage_y) ?
...
{
    if(!is_canvas(id)) return;
    canvasvec[id].r = r; canvasvec[id].g = g; canvasvec[id].b = b;
}
