...
void init_section_annotation_markers()
{
	...
	register_marker_type("Freeform Marker");
		...
		// Get marker texture properly in framework and JNI lib
	
	char* pointMarkerPath;
	char* spanMarkerPath;
	char* outlineMarkerPath;
	char* clastMarkerPath;
	char* sampleMarkerPath;
	char* pvMarkerPath;
	char* freeformMarkerPath;
	
#if defined(__APPLE__) && !defined(JAVA_BINDING)
		// allocate textures from framework bundle
	pointMarkerPath    = getFilePathInBundle("coresection_point_marker",    "png");
	spanMarkerPath     = getFilePathInBundle("coresection_span_marker",     "png");
	outlineMarkerPath  = getFilePathInBundle("coresection_outline_marker",  "png");
	clastMarkerPath    = getFilePathInBundle("coresection_clast_marker",    "png");
	sampleMarkerPath   = getFilePathInBundle("coresection_sample_marker",   "png");
	pvMarkerPath       = getFilePathInBundle("coresection_pv_marker",       "png");
	freeformMarkerPath = getFilePathInBundle("coresection_freeform_marker", "png");
	
#else		
		// allocate textures from application relative path
	const int kBufferSize    = 256;
	
	pointMarkerPath    = (char *) malloc(kBufferSize);
	spanMarkerPath     = (char *) malloc(kBufferSize);
	outlineMarkerPath  = (char *) malloc(kBufferSize);
	clastMarkerPath    = (char *) malloc(kBufferSize);
	sampleMarkerPath   = (char *) malloc(kBufferSize);
	pvMarkerPath       = (char *) malloc(kBufferSize);
	freeformMarkerPath = (char *) malloc(kBufferSize);
	
	strcpy(pointMarkerPath,    "resources/coresection_point_marker.png");
	strcpy(spanMarkerPath,     "resources/coresection_span_marker.png");
	strcpy(outlineMarkerPath,  "resources/coresection_outline_marker.png");	
	strcpy(clastMarkerPath,    "resources/coresection_clast_marker.png");
	strcpy(sampleMarkerPath,   "resources/coresection_sample_marker.png");
	strcpy(pvMarkerPath,       "resources/coresection_pv_marker.png");
	strcpy(freeformMarkerPath, "resources/coresection_freeform_marker.png");
#endif	
    ...
    alloc_marker_type_resource(coresection_marker[ CORE_FREEFORM_MARKER ],
		...
		// Releases
	if(pointMarkerPath)    { free(pointMarkerPath); }
	if(spanMarkerPath)     { free(spanMarkerPath);  }
	if(outlineMarkerPath)  { free(outlineMarkerPath); }
	if(clastMarkerPath)    { free(clastMarkerPath); }
	if(sampleMarkerPath)   { free(sampleMarkerPath); }
	if(pvMarkerPath)       { free(pvMarkerPath); }
	if(freeformMarkerPath) { free(freeformMarkerPath); }
}
